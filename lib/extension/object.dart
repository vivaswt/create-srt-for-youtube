...
typedef CheckFunction<T> = ({bool expect, String message}) Function(T);

extension FutureCheck<T> on Future<T> {
  Future<T> check(CheckFunction<T> convert) => then((r) => r.check(convert));
  Future<(R, S)> fork<R, S>(
    Future<R> Function(T) f,
    Future<S> Function(T) g,
  ) async {
    final r = await then(f);
    final s = await then(g);
    return (r, s);
  }
}
